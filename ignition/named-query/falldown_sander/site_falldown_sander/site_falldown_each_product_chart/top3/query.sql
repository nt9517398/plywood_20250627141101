WITH sander_data AS (
    ...
    where  t_stamp >= :date_from and t_stamp <= :date_to and thickness =:thickness and grade =:grade and product=:product and construction =:construction
),
ranked_data AS (
    SELECT 
        'Harvester Mark' AS label, hm_count, (hm_count::float / row_count) AS percentage FROM sander_data
    UNION ALL
...
SELECT label, hm_count, percentage
FROM (
    SELECT label, hm_count, percentage, 
